...
export default function Footer({ locale, t }) {
  const f = t?.footer || {}
  const base = `/${locale}`
  const hairInv = { borderColor: 'var(--hair-inv)' }

  return (
    <footer>
      <div className="band is-ink is-last pt-12 pb-8">

        {/* meta row */}
        <div className="flex items-center justify-between pb-8 mb-10" style={{ borderBottom: '1px solid var(--hair-inv)' }}>
          <span className="mono text-[10.5px] uppercase tracking-[0.18em] text-gray-500">EU // OPPORTUNITIES</span>
          <span className="mono text-[10.5px] uppercase tracking-[0.18em] text-gray-500">{locale}</span>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-10 mb-10">
          ...
          <div>
            ...
            <p className="text-sm leading-relaxed text-gray-400 mb-4">{f.tagline}</p>
            ...
            <div className="pt-4" style={hairInv}>
          ...
          {/* Program links */}
          <div>
            <p className="mono text-[10.5px] font-medium tracking-[0.18em] uppercase text-gray-500 mb-4">{f.programs}</p>
            <ul className="space-y-1.5">
              {programs.map((p) => (
                <li key={p.slug}>
                  <Link href={`${base}/program/${p.slug}`} className="text-sm text-gray-400 hover:text-white transition-colors">
                    {p.emoji} {p.name}
                  </Link>
                </li>
              ))}
          ...
          {/* Official sources */}
          <div>
            <p className="mono text-[10.5px] font-medium tracking-[0.18em] uppercase text-gray-500 mb-4">{f.sources}</p>
            <ul className="space-y-1.5">
              {[
        ...
        <div className="pt-6 flex flex-col sm:flex-row justify-between gap-2 mono text-[10.5px] uppercase tracking-[0.12em] text-gray-600" style={{ borderTop: '1px solid var(--hair-inv)' }}>
          <p>{f.unofficial}</p>
          <p>{f.date}</p>
        </div>
      </div>
